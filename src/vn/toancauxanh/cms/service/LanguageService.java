...
public class LanguageService extends BasicService<Language> {
	public JPAQuery<Language> getTargetQuery() {
		...
		JPAQuery<Language> q = find(Language.class).where(QLanguage.language.trangThai.ne(core().TT_DA_XOA));
	...
	public List<Language> getListLanguage() {
		...
		list.addAll(find(Language.class).where(QLanguage.language.trangThai.eq(core().TT_AP_DUNG))
				.orderBy(QLanguage.language.title.desc()).fetch());
		if (list.isEmpty()) {
