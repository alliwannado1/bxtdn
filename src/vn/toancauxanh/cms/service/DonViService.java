...
public class DonViService extends BasicService<DonVi> {
	...
	@Init
	public void init() {
		List<DonVi> list = find(DonVi.class).where(QDonVi.donVi.trangThai.ne(core().TT_DA_XOA)).fetch();
	...
	public List<DonVi> getListDonViCha() {
		...
		list = find(DonVi.class).where(QDonVi.donVi.cha.isNull()).orderBy(QDonVi.donVi.ten.asc()).fetch();
		return list;
	}
	...
	public List<DonVi> getListAllDonVi() {
		List<DonVi> list = new ArrayList<DonVi>();
		list.add(null);
		list.addAll(find(DonVi.class).orderBy(QDonVi.donVi.cha.ten.asc()).orderBy(QDonVi.donVi.ten.asc()).fetch());
		return list;
	}
	...
	public List<DonVi> getListDonViCon() {
		...
		if (selectedDonVi != null) {
			list = find(DonVi.class).where(QDonVi.donVi.cha.eq(selectedDonVi)).orderBy(QDonVi.donVi.ten.asc()).fetch();
		}
		return list;
	}
	...
	public List<DonVi> getListDonViConTheoCha(DonVi cha) {
		List<DonVi> list = new ArrayList<DonVi>();
		list = find(DonVi.class).where(QDonVi.donVi.cha.eq(cha)).orderBy(QDonVi.donVi.ten.asc()).fetch();
		return list;
	}
	...
	public JPAQuery<DonVi> getTargetQuery() {
		...
		JPAQuery<DonVi> toDanPho = find(DonVi.class).where(QDonVi.donVi.trangThai.ne(core().TT_DA_XOA));
