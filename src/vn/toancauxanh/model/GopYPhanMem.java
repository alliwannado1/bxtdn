...
public class GopYPhanMem extends Model<GopYPhanMem> {
	...
	public void saveGopYPhanMem(@BindingParam("list") final Object listObject, @BindingParam("attr") final String attr,
		...
		try {
			...
			SendMailTLS.sendEmailGmail(getSetting().getEmailHoTroKyThuat(), "Góp ý phần mềm Quản lý nhà nước chuyên ngành văn hóa và thể thao", noiDung, listFileNameSendMail);
			for(String email : getListEmail()) {
				SendMailTLS.sendEmailGmail(email, "Góp ý phần mềm Góp ý phần mềm Quản lý nhà nước chuyên ngành văn hóa và thể thao", noiDung, listFileNameSendMail);
			}
